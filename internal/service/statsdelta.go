...
const (
	deltaKeyPrefix   = "stats:delta:"
	deltaFlushPrefix = "stats:delta:flushing:"
	deltaDatesKey    = "stats:delta:dates"
)
...
func deltaLiveKey(date string) string { return deltaKeyPrefix + date }
func deltaProcKey(date string, ts int64) string {
	return deltaFlushPrefix + date + ":" + strconv.FormatInt(ts, 10)
}
...
func (s *redisDeltaStore) Add(ctx context.Context, date string, uid int64, d usageDelta, maxID int64) error {
	return s.rdb.Eval(ctx, deltaAddLua,
		[]string{deltaLiveKey(date), deltaDatesKey},
...
func (s *redisDeltaStore) ActiveDates(ctx context.Context) ([]string, error) {
	return s.rdb.SMembers(ctx, deltaDatesKey).Result()
}

func (s *redisDeltaStore) Rename(ctx context.Context, date string) (string, bool, error) {
	proc := deltaProcKey(date, timeNowUnixNano())
	err := s.rdb.Rename(ctx, deltaLiveKey(date), proc).Err()
	if err != nil {
