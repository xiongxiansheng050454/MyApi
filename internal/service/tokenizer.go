...
func newTokenizerCache(fallbackEncoding string) *tokenizerCache {
	...
	return &tokenizerCache{codecs: map[string]tokenizer.Codec{}, fallback: enc}
}

func (t *tokenizerCache) codecFor(model string) tokenizer.Codec {
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (t *tokenizerCache) count(model, text string) int {
	if text == "" {
		return 0
	}
	c := t.codecFor(model)
