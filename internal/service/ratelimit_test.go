...
func ident(userID, keyID int64, overrides map[string]int64) *KeyIdentity {
	return &KeyIdentity{UserID: userID, ApiKeyID: keyID, AllowAll: true, Overrides: overrides}
}
...
func TestGateRejectByOverride(t *testing.T) {
	...
	req := &ChatCompletionRequest{Key: ident(5, 9, map[string]int64{model.MetricRPM: 5}), Model: "gpt-4", Body: []byte(`{}`)}
	base := s.slidingKey(model.TargetGlobal, "*", model.MetricRPM, 60)
	store.denyAlways[base] = true
	_, err := s.rateLimitGate(context.Background(), req)
...
func TestGateQueueWaitsThenAllowed(t *testing.T) {
	...
	base := s.slidingKey(model.TargetUser, "5", model.MetricRPM, 60)
	store.denyCount[base] = 1 // first call denied, second allowed
	req := &ChatCompletionRequest{Key: ident(5, 9, nil), Model: "gpt-4", Body: []byte(`{}`)}
	_, err := s.rateLimitGate(context.Background(), req)
	if err != nil {
...
func TestGateConcurrencyRejectAndRelease(t *testing.T) {
	...
	req := &ChatCompletionRequest{Key: ident(5, 9, nil), Model: "gpt-4", Body: []byte(`{}`)}
	rel, err := s.rateLimitGate(context.Background(), req)
	if err != nil {
	...
	rel2, err := s.rateLimitGate(context.Background(), req)
	if err != nil {
...
func TestRateLimitedErrorMessage(t *testing.T) {
	e := &RateLimitedError{RetryAfter: 7}
	if e.Error() == "" {
