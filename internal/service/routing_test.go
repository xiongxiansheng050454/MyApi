...
func TestOutputBudget(t *testing.T) {
	...
	if b := s.outputBudget([]byte(`{"max_tokens":77}`)); b != 77 {
		t.Fatalf("max_tokens not honored: %d", b)
	}
	if b := s.outputBudget([]byte(`{"max_completion_tokens":33}`)); b != 33 {
		t.Fatalf("max_completion_tokens not honored: %d", b)
	}
	if b := s.outputBudget([]byte(`{}`)); b != config.Default().Billing.DefaultOutputBudget {
		t.Fatalf("default budget not used: %d", b)
	}
}
