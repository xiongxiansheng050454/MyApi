# 一键创建 Issues + 按模块创建并合并 PR（需先安装 gh 并登录：winget install GitHub.cli; gh auth login）
# 用法：pwsh -File scripts/bootstrap-github.ps1
...
$iDB = New-Issue "[数据库] 表结构设计" @"
用户、Key、渠道、渠道模型、定价、限流规则、用量日志、日汇总、余额、资金流水。
详见 `数据库表设计.txt`。
"@
...
$mods = @(
    ...
    @{ title = "[模块] M5 定价管理"; body = "渠道×模型单价 CRUD 与 upsert。" },
...
$prs = @(
    ...
    @{ head = "feature/05-pricing";         title = "feat: 渠道×模型定价";                  issue = $modIssue["[模块] M5 定价管理"] },
...
foreach ($p in $prs) {
    ...
    gh pr create --base master --head $p.head --title $p.title --body ("Closes #" + $p.issue + "`n`n关联需求 #" + $iReq + " / 架构 #" + $iArch + " / 数据库 #" + $iDB)
