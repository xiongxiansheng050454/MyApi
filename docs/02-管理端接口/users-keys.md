...
| 状态 | 效果 |
| --- | --- |
| `suspended` | 停用：该用户下所有 Key 立即拒绝调用（`403 account_suspended`），数据保留、可恢复 |
| `deleted` | 逻辑删除，拒绝调用；历史账单保留（推荐改用 `suspended`） |
| `active` | 恢复 |
...
**永久删除**用户及其关联数据（网关 Key、余额、资金流水、用量日志、日汇总），不可恢复。仅当确实要清除账号时使用；若只想禁止调用请用 `suspended`。

```json
{ "id": 1001 }
```

> 逻辑删除/停用请使用 §1.5 的 `status`；永久删除会连同用量与账单记录一并清除。
